...
export function MonitorForm({ onSave, onCancel, editingCheck }: MonitorFormProps) {
  const { data: groups = [] } = useGroups();
  const { data: tags = [] } = useTags();
  const { data: settings } = useSettings();
  const hasTailscale = !!settings?.tailscale_api_key && !!settings?.tailscale_tailnet;
  const {
  ...
  useEffect(() => {
    if (editingCheck) {
      form.reset({
        ...
        tailscale_service_protocol: editingCheck.tailscale_service_protocol || 'http',
    ...
    >
          ...
          {(field) => (
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {checkTypes.map((ct) => (
                ...
                >
                  <div className={`text-${ct.color} font-bold mb-1`}>{ct.label}</div>
                  <div className="text-[10px] text-terminal-muted">{ct.description}</div>
                </button>
              ))}
            </div>
          )}
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <form.AppField
          name="name"
          children={(field) => (
            <field.TextField label="Display Name" placeholder="My Server" required />
          )}
        />
        <div className="grid grid-cols-4 gap-4">
          ...
          <form.AppField
            ...
            children={(field) => <field.NumberField label="Timeout (s)" min={1} required />}
          />
          <form.AppField
            name="retries"
            children={(field) => <field.NumberField label="Retries" min={0} max={10} />}
          />
          <form.AppField
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        ...
        <form.AppField
          name="tag_ids"
          children={(field) => <field.TagsField label="Tags" tags={tags} />}
        />
      </div>
        ...
        {(type) => (
          <>
            {(type === 'http' || type === 'json_http') && (
              <HTTPFields form={form} isJsonHttp={type === 'json_http'} />
            )}
          ...
          {(field: any) => (
            <div>
              ...
              <input
                ...
                onChange={(e) => field.handleChange(parseStatusCodes(e.target.value))}
    ...
    <div className="space-y-6 mb-8 p-6 bg-terminal-surface border border-terminal-border rounded-sm">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <form.AppField
          name="dns_hostname"
          children={(field: any) => (
            <field.TextField label="Hostname" placeholder="example.com" required />
          )}
        />
        <form.AppField
      ...
      <form.AppField
        ...
        children={(field: any) => (
          <field.TextField label="Expected Value (Optional)" placeholder="1.2.3.4" />
        )}
      />
    </div>
  );
      ...
      <div className="pt-4 border-t border-terminal-border">
        ...
        <div className="space-y-4">
          <form.AppField
            name="postgres_query"
            children={(field: any) => (
              <field.TextAreaField label="SQL Query" placeholder="SELECT 1" rows={2} />
            )}
          />
          <form.AppField
      ...
      <div>
        ...
        {tailscaleDevices.length > 0 ? (
          ...
          <form.AppField
            name="tailscale_service_host"
            children={(field: any) => (
              <field.TextField label="" placeholder="Device hostname" required />
            )}
          />
        )}
      </div>
          ...
          {(protocol: string) =>
            (protocol === 'http' || protocol === 'https') && (
              <form.AppField
                ...
                children={(field: any) => <field.TextField label="Path" placeholder="/" />}
              />
            )
          }
        </form.Subscribe>
